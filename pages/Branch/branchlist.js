...
import { confirnDeleteAction, editDepartmentAction, editEmployeeAction, editBranchData, getDepartmentList, getPositionList, editBranchAction, getbranchList } from '@/store/authSlice';
...
const DepartmentListPage = () => {
  ...
  const [dropdownOptions, setDropdownOptions] = useState({
    ...
    ordering: [
      { id: 'area_code', label: 'Branch Code', sortable: true },
      { id: 'area_name', label: 'Branch Name', sortable: true },
      { id: 'parent_area', label: 'parent Position', sortable: true },
      // { id: 'department', label: 'department', sortable: true },
      // { id: 'position', label: 'Position', sortable: true },
      // { id: 'mobile', label: 'Mobile', sortable: true },
      // { id: 'email', label: 'Email', sortable: true },
      // { id: 'parent_area_name', label: 'Parent Branch Name', sortable: true },
      ...
      // { id: '1', label: 'Branch', sortable: true },
  ...
  const columns = [
    ...
    { id: 'area_code', label: 'Branch Code', sortable: true },
    { id: 'area_name', label: 'Branch Name', sortable: true },
    { id: 'parent_area', label: 'parent Branch Code', sortable: true },
  ...
  const fetchEmployees = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:7000/area/getAreaList`, {
  ...
  useEffect(() => {
    fetchEmployees();
  }, [pagination.page, pagination.page_size, sorting, search, filters,deletepopup.editBranchData]);
  ...
  const handleMenuClick = (event, employee) => {
    ...
    setSelectedEmployee({...employee,parent_area:employee?.parent_area?.id});
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      let deleteresponse=await axios.post(`http://localhost:7000/area/actionArea`, data, {
        headers: { Authorization: token }
      });
        if(deleteresponse?.data.detail){
          setOpenSnackbar("deleted successfully")
          fetchEmployees()
          // setOpenSnackbar(deleteresponse?.data.detail)

        }
        else{
          setOpenSnackbar({status:true,message:"deleted successfully"})

          setSelected([]);
          setShowDelete(false);
          dispatch(confirnDeleteAction(false));
          setOpen("deleteposition");
          fetchEmployees();
        }
  ...
  positions = useMemo(() => {
    if (getBranchListData?.length) {
     ...
     useEffect(()=>{
      ...
      console.log(getBranchListData,"#############################11")
                  ...
                  employees.map((employee, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>{employee?.area_code}</TableCell>
                        <TableCell>{employee?.area_name}</TableCell>
                        <TableCell>{employee?.parent_area?.area_code}</TableCell>
                   
                        <TableCell>
                          {employee?.parent_area?.area_name ?  employee?.parent_area?.area_name : '-'}
                        </TableCell>
                         <TableCell>
                                                  <IconButton 
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} sm={6} md={4}>
              <TextField
                ...
                onChange={(e) => handleFilterChange('area_name', e.target.value)}
              />
            </Grid>

            {console.log(filters,"@@@@@@@#############",positions)}
            <Grid item xs={12} md={6}>
                      <TextField
                        ...
                        name="parent_area"
                        value={filters?.parent_area || ''}
                        variant="outlined"
                        onChange={(e) => {handleFilterChange('parent_area', e.target.value)}}
            ...
            <Grid item xs={12} sm={6} md={4}>
              <FormControl fullWidth>
                ...
                >
                  {dropdownOptions.ordering.map((position) => {
                    return(
