...
export default function LeaveManagement() {
 ...
 const [viewTypepast, setviewTypepast] = useState('past');
  ...
  const transformLeavesData = (data) => {
  const leaveTypes = [
    {
      id: 'casual',
      name: 'Casual Leave',
    ...
    const getLeaves = async (id) => {
      try {
       if(true){
        ...
        //  setleavetypesdata()
        const leavesResponse = deleteresponse.data;

const leaveTypes = transformLeavesData(leavesResponse);

console.log(leaveTypes);
setleavetypesdata(leaveTypes);
       }
       else{
    ...
    console.log(leavetypesdata,"leavetypesdata9999999999999999")
useEffect(()=>{
getLeaves()
},[])
...
<LeaveManagementTabs />
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        {leavetypesdata.length && leavetypesdata.map((leave) => (
          ...
          title={
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
              ...
              >
                ...
                <MenuItem value="past">Upcoming Leave</MenuItem>
                <MenuItem value="upcoming">Upcoming Holidays</MenuItem>
              </Select>
              {/* {viewType === 'upcoming' && (
                <Select
                  value={viewSubType}
        ...
        <CardHeader
          title={
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
              <Select
                value={viewTypepast}
                onChange={(e) => setviewTypepast(e.target.value)}
                size="small"
                sx={{ mr: 2, minWidth: 200 }}
              >
                <MenuItem value="past">Past Leave & Holidays</MenuItem>
               <MenuItem value="past">Past Leaves</MenuItem>
              <MenuItem value="past">Past Holidays</MenuItem>
