...
import { confirnDeleteAction, editDepartmentAction, editEmployeeAction, editPositionAction, getDepartmentList, getPositionList } from '@/store/authSlice';
...
import ViewPositionModal from '../../components/Dashboard/viewposition';
...
const DepartmentListPage = () => {
  ...
  const {getPositionListData } = useSelector(state => state.auth);
  ...
  const [dropdownOptions, setDropdownOptions] = useState({
    ...
    ordering: [
      { id: 'position_code', label: 'Position code', sortable: true },
      { id: 'position_name', label: 'Position Name', sortable: true },
      { id: 'parent_position', label: 'parent Position', sortable: true },
      { id: 'department', label: 'department', sortable: true },
      { id: 'position', label: 'Position', sortable: true },
      { id: 'mobile', label: 'Mobile', sortable: true },
      { id: 'email', label: 'Email', sortable: true },
      { id: 'parent_position_name', label: 'Parent Position Name', sortable: true },
  ...
  const columns = [
    ...
    { id: 'position_code', label: 'Position Code', sortable: true },
    { id: 'position_name', label: 'Position Name', sortable: true },
    { id: 'parent_position', label: 'parent Position code', sortable: true },
  ...
  useEffect(() => {
    fetchEmployees();
  }, [pagination.page, pagination.page_size, sorting, search, filters,deletepopup.editPositionData]);
  ...
  const handleMenuClick = (event, employee) => {
    ...
    setSelectedEmployee({...employee,parent_position:employee?.parent_position?.id});
  };
  ...
  const handleEditSubmit = async (values) => {
    try {
      dispatch(editPositionAction(values));
...
setTimeout(() => {
                  ...
                  employees.map((employee, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>{employee?.position_code}</TableCell>
                        <TableCell>{employee?.position_name}</TableCell>
                        <TableCell>{employee?.parent_position?.position_code}</TableCell>
                   
                        <TableCell>
                          {employee?.parent_position_name ? employee.parent_position_name : '-'}
                        </TableCell>
                         <TableCell>
                                                  <IconButton 
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={6}>
                      <TextField
                        ...
                        value={filters.parent_position.id}
                        variant="outlined"
                      >
  ...
  >
    <Box sx={{
      ...
      <Typography variant="h5" gutterBottom>Edit Position</Typography>
      <Divider sx={{ mb: 3 }} />
      <Formik
      ...
      >
        {({ values, errors, touched, handleChange, setFieldValue }) => (
          <Form>
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12}>
                <Typography variant="h6" gutterBottom>Position Information</Typography>
                <Divider />
              </Grid>
       ...
       <Grid item xs={12} md={6}>
        ...
        {/* Error Snackbar */}
        {console.log(openSnackbar,"8777777777777777")}
