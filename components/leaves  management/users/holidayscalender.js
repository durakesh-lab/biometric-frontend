...
const HolidaysByCalenderPage = ({holidays}) => {
  ...
  const events = holidays.flatMap(holiday => {
    return holiday.dates.map((date, index) => ({
      id: `${index+1}`,
      title: holiday.name,
      start: date,
      end: date,
      allDay: true,
      backgroundColor: holiday.color, // Changed from 'color' to 'backgroundColor'
      extendedProps: {
        type: 'holiday',
...
export default HolidaysByCalenderPage;